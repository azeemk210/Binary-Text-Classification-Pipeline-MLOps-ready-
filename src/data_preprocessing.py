...
def transform_text(text):
    ...
    #logger.debug("tokenized the text")
    text = [word for word in text if word.isalnum()] # Remove non-alphanumeric tokens
    #logger.debug("Remove non-alphanumeric tokens")
    text = [word for word in text if word not in stopwords.words('english') and word not in string.punctuation] # Remove words and punctuation
    #logger.debug("Remove words and punctuation")
    text = [ps.stem(word) for word in text] # stem the words
    #logger.debug("stem the words")
    return " ".join(text)
...
def prepocess_df(df, text_column='text', target_column='target'):
    ...
    try:
        ...
        logger.debug('Duplicate rows removes')
